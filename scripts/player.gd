...
# Draws as a tiny CRT monitor with a face.
...
func _draw() -> void:
	var alpha := 0.45 if _invuln_t > 0.0 and int(_invuln_t * 20) % 2 == 0 else 1.0
	# Beige CRT monitor body.
	...
	# Screen (player color).
	var sc := Color(color.r, color.g, color.b, alpha)
	draw_rect(Rect2(Vector2(-11, -12), Vector2(22, 18)), sc, true)
	# Face: two dot eyes + mouth.
	draw_rect(Rect2(Vector2(-7, -7), Vector2(2, 2)), Color(0,0,0,alpha), true)
	draw_rect(Rect2(Vector2(5, -7), Vector2(2, 2)), Color(0,0,0,alpha), true)
	draw_rect(Rect2(Vector2(-4, -2), Vector2(8, 2)), Color(0,0,0,alpha), true)
	# Power LED.
...
func apply_pickup(kind: int) -> void:
	# Kind enum matches pickup.gd: 0 CD, 1 MODEM, 2 FLOPPY_STACK.
	match kind:
		0:
			_pierce_ammo = PICKUP_PIERCE_SHOTS
		1:
			_speed_boost_t = PICKUP_DURATION_SPEED
		2:
			_spread_ammo = PICKUP_SPREAD_SHOTS
